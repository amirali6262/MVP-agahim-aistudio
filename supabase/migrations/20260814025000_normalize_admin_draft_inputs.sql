...
create or replace function public.create_obligation_draft(
  requested_family_id uuid,
  requested_code text,
  requested_title text,
  requested_summary text default null,
  requested_authority_name text default null,
  requested_official_action_url text default null,
  requested_legal_reference text default null,
  requested_source_url text default null,
  requested_effective_from date default null,
...
declare
  ...
  official_url text:=nullif(btrim(requested_official_action_url),'');
  source_url text:=nullif(btrim(requested_source_url),'');
begin
  if uid is null
  ...
  if official_url is not null and official_url!~'^https://'
     or source_url is not null and source_url!~'^https://' then
  ...
  insert into public.obligations(
    ...
    requested_family_id,requested_code,btrim(requested_title),
    nullif(btrim(requested_summary),''),
    nullif(btrim(requested_authority_name),''),
    official_url,uid
  ...
  insert into public.obligation_versions(
    ...
    created_obligation.id,1,'DRAFT',nullif(btrim(requested_legal_reference),''),
    source_url,requested_effective_from,requested_recurrence_rule,
    requested_deadline_rule,requested_penalty_rule,uid
