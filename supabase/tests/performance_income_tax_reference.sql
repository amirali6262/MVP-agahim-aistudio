...
begin
  if not exists (
    ...
    where o.code='PERFORMANCE_INCOME_TAX' and o.is_active and v.status='PUBLISHED'
  ...
  from unnest(array['PIT-001','PIT-002','PIT-003','PIT-004','PIT-005','PIT-010','PIT-011','PIT-012',
    'PIT-020','PIT-021','PIT-022','PIT-023','PIT-024','PIT-025','PIT-026','PIT-027',
    'PIT-030','PIT-031','PIT-032','PIT-040','PIT-050','PIT-051']) required_code
  ...
  if exists(select 1 from public.workflow_steps s where s.workflow_template_id=workflow_id
    and s.code not in ('PIT-040','PIT-051')
...
do $$ begin
  ...
  if exists(select 1 from public.workflow_transitions t
    ...
    where s.code='PIT-030' and t.outcome_code='LEGAL_SERVICE_DEEMED_OBJECTION'
  ...
  if exists(select 1 from public.workflow_transitions t
    join public.workflow_steps s on s.id=t.from_step_id
    where s.code='PIT-027' and t.outcome_code='NO_RESPONSE'
