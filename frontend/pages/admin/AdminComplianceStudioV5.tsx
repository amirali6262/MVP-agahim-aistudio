...
export default function AdminComplianceStudio() {
  ...
  const editRule = async (rule: RuleSet) => {
    const title = window.prompt('عنوان قاعده', rule.title)
    if (title == null || !title.trim()) return
    const explanation = window.prompt('توضیح قاعده', rule.explanation ?? '')
    if (explanation == null) return
    const { error } = await supabase.from('eligibility_rule_sets').update({ title: title.trim(), explanation: explanation.trim() }).eq('id', rule.id)
    if (error) return toast.error(error.message)
    toast.success('قاعده ویرایش شد.'); await loadDefinition()
  }

  const deleteRule = async (rule: RuleSet) => {
    if (!window.confirm(`قاعده «${rule.title}» حذف شود؟`)) return
    const { error } = await supabase.from('eligibility_rule_sets').delete().eq('id', rule.id)
    if (error) return toast.error(error.message)
    toast.success('قاعده حذف شد.'); await loadDefinition()
  }

  const editStep = async (step: WorkflowStep) => {
    const title = window.prompt('عنوان مرحله', step.title)
    if (title == null || !title.trim()) return
    const instructions = window.prompt('راهنمای مرحله', step.instructions ?? '')
    if (instructions == null) return
    const { error } = await supabase.from('workflow_steps').update({ title: title.trim(), instructions: instructions.trim() }).eq('id', step.id)
    if (error) return toast.error(error.message)
    toast.success('مرحله ویرایش شد.'); await loadDefinition()
  }

  const deleteStep = async (step: WorkflowStep) => {
    if (!window.confirm(`مرحله «${step.title}» و مسیرهای وابسته حذف شود؟`)) return
    const { error } = await supabase.rpc('delete_workflow_step_definition', { requested_step_id: step.id })
    if (error) return toast.error(error.message)
    toast.success('مرحله حذف شد.'); await loadDefinition()
  }
  ...
  const deleteItem = async (item: CatalogItem) => {
    ...
    try {
      if (isSupabaseConfigured) {
        const { error } = await supabase.rpc('delete_obligation_definition', { requested_obligation_id: item.obligation.id })
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      ...
      setFamilies(familyRows)
      const uniqueObligations = Array.from(new Map((obligationResult.data ?? []).map((obligation) => [obligation.id, obligation])).values())
      const cat = uniqueObligations.map((obligation) => ({
        obligation,
        family: familyRows.find((family) => family.id === obligation.family_id) ?? null,
        versions: versionRows.filter((version) => version.obligation_id === obligation.id),
      }))
            ...
            <>
              <div className="rounded-2xl border border-zinc-800 bg-[#101211] p-6 shadow-xl shadow-black/10">
                ...
                <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
                  ...
                  <Metric icon={Clock3} label="مهلت و دوره" value={deadlineLabel(selectedVersion.deadline_rule, selectedVersion.recurrence_rule)} />
                </div>

                <DeadlineEditor version={selectedVersion} editable={mode === 'EDIT' && selectedVersion.status === 'DRAFT'} onSaved={async () => { await loadCatalog(); await loadDefinition() }} />
                    ...
                    {rules.length === 0 ? (
                      ...
                      rules.map((rule, idx) => (
                        <div key={rule.id ?? idx} className="rounded-xl border border-zinc-700/80 bg-[#1b1e1c] p-4 space-y-3 shadow-sm">
                          ...
                          {mode === 'EDIT' && selectedVersion.status === 'DRAFT' && <div className="flex gap-2 border-t border-zinc-800 pt-3"><Button size="sm" variant="outline" onClick={() => void editRule(rule)} className="gap-1"><Pencil className="h-3.5 w-3.5" />ویرایش</Button><Button size="sm" variant="ghost" onClick={() => void deleteRule(rule)} className="gap-1 text-red-400"><Trash2 className="h-3.5 w-3.5" />حذف</Button></div>}
                        </div>
                      ))
                    )}
                  </div>
                    ...
                    {steps.length === 0 ? (
                      ...
                      steps.map((step, idx) => (
                        <div key={step.id ?? idx} className="rounded-xl border border-zinc-700/80 bg-[#1b1e1c] p-4 space-y-2.5 shadow-sm">
                          ...
                          {mode === 'EDIT' && selectedVersion.status === 'DRAFT' && <div className="flex gap-2 border-t border-zinc-800 pt-3"><Button size="sm" variant="outline" onClick={() => void editStep(step)} className="gap-1"><Pencil className="h-3.5 w-3.5" />ویرایش</Button><Button size="sm" variant="ghost" onClick={() => void deleteStep(step)} className="gap-1 text-red-400"><Trash2 className="h-3.5 w-3.5" />حذف</Button></div>}
                        </div>
                      ))
                    )}
                  </div>
              ...
              <div className="rounded-2xl border border-zinc-800 bg-[#101211] p-6 shadow-xl shadow-black/10">
                ...
                {!transitionSchemaReady && <p className="mt-4 rounded-lg bg-amber-950/40 p-3 text-xs text-amber-200">جدول مسیرها هنوز روی Supabase مقصد اعمال نشده است. فرم برای مشاهده گزینه‌ها فعال است؛ ذخیره پس از اجرای migration امکان‌پذیر می‌شود.</p>}
                {mode === 'EDIT' && selectedVersion.status === 'DRAFT' && (
...
function DeadlineEditor({ version, editable, onSaved }: { version: Version; editable: boolean; onSaved: () => Promise<void> }) {
  const current = version.deadline_rule && !Array.isArray(version.deadline_rule) && typeof version.deadline_rule === 'object' ? version.deadline_rule : {}
  const [editing, setEditing] = useState(false)
  const [base, setBase] = useState(String(current['base'] ?? 'FISCAL_YEAR_END'))
  const [months, setMonths] = useState(String(current['gap_months'] ?? '4'))
  const save = async (remove = false) => {
    const gap = Number(months)
    if (!remove && (!Number.isInteger(gap) || gap < 0 || gap > 120)) return toast.error('فاصله مهلت باید عددی بین صفر تا ۱۲۰ ماه باشد.')
    const { error } = await supabase.from('obligation_versions').update({
      recurrence_rule: remove ? {} : { frequency: 'YEARLY' },
      deadline_rule: remove ? {} : { base, gap_months: gap },
    }).eq('id', version.id)
    if (error) return toast.error(error.message)
    toast.success(remove ? 'مهلت و دوره حذف شد.' : 'مهلت و دوره ذخیره شد.')
    setEditing(false); await onSaved()
  }
  return <section className="mt-5 rounded-2xl border border-sky-900/50 bg-gradient-to-l from-sky-950/30 to-zinc-950/30 p-5">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-sky-950 text-sky-300"><Clock3 className="h-5 w-5" /></span><div><h4 className="font-bold">زمان‌بندی و دوره اجرا</h4><p className="mt-1 text-xs text-zinc-400">مبنای محاسبه و فاصله سررسید این نسخه را مدیریت کنید.</p></div></div>{editable && <div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setEditing((v) => !v)}><Pencil className="ml-1 h-3.5 w-3.5" />{editing ? 'بستن' : 'ویرایش'}</Button><Button size="sm" variant="ghost" className="text-red-400" onClick={() => window.confirm('مهلت و دوره حذف شود؟') && void save(true)}><Trash2 className="ml-1 h-3.5 w-3.5" />حذف</Button></div>}</div>
    {editing && <div data-studio-dirty="true" className="mt-5 grid gap-4 rounded-xl border border-zinc-800 bg-zinc-950/40 p-4 md:grid-cols-3"><Field label="مبنای مهلت"><Select value={base} onValueChange={setBase}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="FISCAL_YEAR_END">پایان سال مالی</SelectItem><SelectItem value="EVENT_DATE">تاریخ وقوع رویداد</SelectItem><SelectItem value="NOTICE_DATE">تاریخ ابلاغ</SelectItem></SelectContent></Select></Field><Field label="فاصله تا سررسید (ماه)"><Input type="number" min="0" max="120" value={months} onChange={(e) => setMonths(e.target.value)} /></Field><div className="flex items-end"><Button className="w-full bg-sky-700 hover:bg-sky-600" onClick={() => void save()}>ذخیره زمان‌بندی</Button></div></div>}
  </section>
}
...
function FamilyForm({ onSaved, onDirtyChange }: { onSaved: () => Promise<void>; onDirtyChange: (dirty: boolean) => void }) {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.rpc('create_obligation_family', { requested_code: code.trim(), requested_title: title.trim(), requested_domain: domain })
    if (error) {
      toast.error(`ثبت گروه انجام نشد: ${error.message}`)
      return
    }
    toast.success('گروه ثبت شد.')
    await onSaved()
...
function deadlineLabel(deadline: Json, recurrence: Json) { const d = deadline && !Array.isArray(deadline) && typeof deadline === 'object' ? deadline : {}; const r = recurrence && !Array.isArray(recurrence) && typeof recurrence === 'object' ? recurrence : {}; if (!Object.keys(d).length) return 'تعریف نشده'; const base = ({ FISCAL_YEAR_END: 'پایان سال مالی', EVENT_DATE: 'تاریخ رویداد', NOTICE_DATE: 'تاریخ ابلاغ' } as Record<string, string>)[String(d['base'])] ?? 'مبنای سفارشی'; const frequency = r['frequency'] === 'YEARLY' ? 'سالانه' : 'دوره سفارشی'; return `${frequency} · ${Number(d['gap_months'] ?? 0).toLocaleString('fa-IR')} ماه پس از ${base}` }
